...
var addTabs = function(obj) {
	...
	if(!$("#" + id)[0]) {
		...
		if(obj.content) {
			...
			content = '<div role="tabpanel" class="tab-pane" id="' + id + '"><style>iframe{width:100%}</style><iframe  src="' + obj.url + '?id=' + obj.id + '" id="iframepage_' + obj.id + '" width="100%" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes"></iframe><script>iFrameResize()</script></div>';
...
$("#logout").click(function() {
	...
	$.ajax({
		type: "POST",
		url: "/Photo/admin/logout",
		async: false,
		success: function(response) {
			window.location.href = baseUrl + "/Photo/manage/login.html"
		}
	});
});
$(function() {
