...
require(['aceEle','zDialog','jqtips','jqueryForm','jqValidate','datepicker','tag','chosen'],function() {
	...
	var pabcI = getQueryStr("pabcI"); 
	 ...
	 $.post("/Photo/getAllLabel",function(data) {
		 if(data.status==0){
			$.each(data.data, function(i, item) {
				var option = $("<option>").val(item.id).text(item.name);
				$("#labelId").append(option);
			});
				$("#labelId").chosen({allow_single_deselect : true,search_contains:true}).trigger("chosen:updated");
		 }
		});
	 $.post("/Photo/getAllLevel",function(data) {
		 if(data.status==0){
			 $.each(data.data, function(i, item) {
				 var option = $("<option>").val(item.id).text(item.name);
				 $("#levelId").append(option);
			 });
			 $("#levelId").chosen({allow_single_deselect : true,search_contains:true}).trigger("chosen:updated");
		 }
	 });
	 $.post("/Photo/getAllSpots",function(data) {
		 if(data.status==0){
			 $.each(data.data, function(i, item) {
				 var option = $("<option>").val(item.id).text(item.name);
	...
	$('#addForm').validate({
		...
		submitHandler: function (form) {
			$("#addForm").ajaxSubmit({
				...
				type : "post",
				url : "/Photo/addPhotographer", // 提交的URL 
